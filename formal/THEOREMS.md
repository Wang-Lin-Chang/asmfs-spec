...
**证明**：N ∈ {2,3,4} 全交错穷举（去重状态 13/55/217），断言 doneWins = 1 且退出码单值。✓
...
**证明**：pid ∈ {0,1,2} × startSec ∈ {⊥,100,200} × 活/死/复用全枚举 36 例：三分唯一且正确；pid_reused 分支在 N ∈ {2,3,4} 并发收养下全部拒绝（0 赢家）。✓
...
**证明**：2⁵ = 32 个标记子集 + 空集 = 33 例穷举：observe 全部唯一且等于最高优先级标记。✓
...
```bash
node bmc-checker.mjs        # 全部 6 条
node bmc-checker.mjs E-4    # 单条（含证伪反例 + 修复版对照）
```

每条定理的规约（陈述/前提/公理/装置锚/边界）在 `specs/E-*.json`。
每台装置的结果与诚实边界在 `E-REPORT.md`。

## 诚实边界（总）

1. 有界穷举（N ≤ 4、有限状态空间）是**组合证明**，不是全称证明；N > 4 与无限域不声称；
